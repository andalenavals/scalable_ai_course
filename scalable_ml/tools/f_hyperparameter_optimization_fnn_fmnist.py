...
from scalable_ml.fifth_exercise.ml_models import FullyConnectedNeuralNetwork
...
OUTPUT_PATH='../output/fmnist_classification_fullyconnectedNN_finetune'
...
def train_and_validate(config, save=False, checkpoint_dir=None):
    ...
    noutput=10
    ml_model = FullyConnectedNeuralNetwork(ninput_feutures,noutput,config["nr"], config["ly"], config["dr"])
    ml_model.to(device)
...
def train_best(path=None, bestconfig=None):
    ...
    test_fmnist_data = FMNISTDataset(test_images, test_targets)
    ...
    ninput_feutures=28*28
    noutput=10
    ml_model = FullyConnectedNeuralNetwork(ninput_feutures,noutput,bestconfig["nr"], bestconfig["ly"], bestconfig["dr"])
